...
class Environment(object):
    ...
    #function that takes unknown arguments because input can be dummy or working agent.
    def create_agent(self, agent_class, *args, **kwargs):   #*args for unknown no. of arguments and **kwargs for item names
        agent = agent_class(self, *args, **kwargs)
        self.agent_states[agent] = {'location': random.choice(self.intersections.keys()), 'heading': (0, 1)} #put agent at random location heading to south
        return agent
    ...
    def reset(self):
        ...
        for agent in self.agent_states.iterkeys():
            ...
            agent.reset(destination=(destination if agent is self.primary_agent else None)) # calls agent reset() function
    ...
    def step(self):
        ...
        # Update agents
        for agent in self.agent_states.iterkeys():
            agent.update(self.t) # calls agents update
    ...
    #sense position of other agents and light
    def sense(self, agent):
        ...
        #green if (state is true (i.e. NS dir) and heading NS) or (state is false(EW) and heading EW)) 
        ...
        for other_agent, other_state in self.agent_states.iteritems():
            #if agent itself or other_agent is at a diff location or other_agent heading in same dir as agent, do nothing
            ...
            if (heading[0] * other_state['heading'][0] + heading[1] * other_state['heading'][1]) == -1: # if other_agent in opposite dir (S if N, W if E...) 0+-1 = -1
                ...
                if left != 'forward':  # we don't want to override left == 'forward' as other agents going left do not affect us
                    left = other_heading
    ...
    #rewards and checking if move is valid
    def act(self, agent, action):
        ...
        if action == 'forward':
            ...
            if light == 'green' or sense['left'] != 'straight':
                heading = (-heading[1], heading[0]) #store new heading in 'heading'
            else:
                move_okay = False
